...
export default function SidebarMobile() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 h-full max-h-screen w-[65vw] bg-white border-r z-40 px-6 py-6 shadow-md md:hidden flex flex-col justify-between transition-transform duration-500 ease-in-out ${isExpanded ? "translate-x-0" : "-translate-x-full"
          }`}>
            ...
            <nav className="flex flex-col gap-4">
              {navItems.map(({ name, path, icon, iconActive }) => (
                <NavLink
                  ...
                  to={path}
                  onClick={() => setIsExpanded(false)}
                  className={`flex items-center gap-3 px-4 py-2 rounded-md ${isActive(path) ? "bg-zinc-200 font-semibold text-black" : "text-zinc-500"
                    }`}>
